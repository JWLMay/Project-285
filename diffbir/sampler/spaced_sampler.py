...
class SpacedSampler(Sampler):
    ...
    def p_sample(
        ...
        if eta == 0.0:
            # DDIM deterministic update (no stochastic noise)
            # x_{t-1} = sqrt(a_{t-1}) * pred_x0 + sqrt(1 - a_{t-1}) * eps_theta
            sqrt_alpha_cumprod_prev = extract_into_tensor(
                torch.cat([torch.ones(1, device=x.device), self.sqrt_alphas_cumprod[:-1]]), t, x.shape
            )
            sqrt_one_minus_alpha_cumprod_prev = torch.sqrt(
                torch.clamp(1.0 - sqrt_alpha_cumprod_prev ** 2, min=0.0)
            )
            # reconstruct eps from pred_x0
            sqrt_recip = extract_into_tensor(self.sqrt_recip_alphas_cumprod, t, x.shape)
            sqrt_recipm1 = extract_into_tensor(self.sqrt_recipm1_alphas_cumprod, t, x.shape)
            eps = (sqrt_recip * x - pred_x0) / sqrt_recipm1
            nonzero_mask = (t != 0).float().view(-1, *([1] * (len(x.shape) - 1)))
            x_prev = (
                sqrt_alpha_cumprod_prev * pred_x0
                + nonzero_mask * sqrt_one_minus_alpha_cumprod_prev * eps
            )
        else:
            # DDPM stochastic update (original)
            mean, variance = self.q_posterior_mean_variance(pred_x0, x, t)
            noise = torch.randn_like(x)
            nonzero_mask = (t != 0).float().view(-1, *([1] * (len(x.shape) - 1)))
            x_prev = mean + nonzero_mask * eta * torch.sqrt(variance) * noise
