...
def save_loss_curve(
    ...
    epoch_points: List[Tuple[int, float]],
    save_path: str,
) -> None:
    try:
        import importlib
    ...
    if len(step_points) == 0 and len(epoch_points) == 0:
        return

    plt.figure(figsize=(10, 5))

    if len(step_points) > 0:
        step_x = [x for x, _ in step_points]
        step_y = [y for _, y in step_points]
        plt.plot(step_x, step_y, label="step loss", linewidth=1.5)

    if len(epoch_points) > 0:
        epoch_x = [x for x, _ in epoch_points]
        epoch_y = [y for _, y in epoch_points]
        plt.plot(epoch_x, epoch_y, label="epoch loss", linewidth=2.0)
...
def main(args) -> None:
    ...
    while global_step < max_steps:
        ...
        for batch in loader:
            ...
            if global_step % cfg.train.log_every == 0 and global_step > 0:
                ...
                if accelerator.is_main_process:
                    ...
                    step_loss_points.append((global_step, avg_loss))
                    save_loss_curve(step_loss_points, epoch_loss_points, loss_curve_path)
        ...
        if accelerator.is_main_process:
            ...
            epoch_loss_points.append((global_step, avg_epoch_loss))
            save_loss_curve(step_loss_points, epoch_loss_points, loss_curve_path)
