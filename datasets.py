...
class StaticPairedData(data.Dataset):
    def __init__(self, hazy_folder, clean_folder, crop_size=448):
        ...
        self.hazy_names = [
            os.path.relpath(name, hazy_folder)
            for name in glob.glob(os.path.join(hazy_folder, '**/*.jpg'), recursive=True)
        ]
        self.crop_size = crop_size
    ...
    def __getitem__(self, index):
        ...
        clean_name = os.path.basename(hazy_name).split('_')[0] + '.jpg'
        clean = cv2.imread(os.path.join(self.clean_folder, clean_name))
