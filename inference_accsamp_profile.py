...
def main(args) -> None:
    ...
    # Read tau / omega from config, allow CLI override
    tau   = args.tau   if args.tau   is not None else cfg.inference.tau
    omega = args.omega if args.omega is not None else cfg.inference.omega
    ...
    print(f"[Profile] tau={tau}, omega={omega}, steps={args.steps}\n")

    # ── Warm-up (not timed) ───────────────────────────────────────────────
    if args.warmup > 0:
        print(f"[Profile] Warm-up: {args.warmup} step(s) with plain sampler.sample() …")
        dummy_img = torch.rand(1, 3, 512, 512, device=device)
        for _ in range(args.warmup):
            cond_w = cldm.prepare_condition(dummy_img, ['remove dense fog'])
            sampler.sample(
                model=cldm, device=device, steps=args.steps,
                x_size=cond_w['c_img'].shape, cond=cond_w,
                uncond=None, cfg_scale=1., progress=False,
            )
        torch.cuda.synchronize(device)
    ...
    times_preprocess    = []
    times_cond          = []
    times_phase1        = []   # accsamp phase-1 (first diffusion pass up to tau)
    times_align         = []   # tiled_align + transmission + VAE encode inside accsamp
    times_phase2        = []   # accsamp phase-2 (guided diffusion pass)
    times_accsamp_total = []   # full accsamp() wall time
    times_vae_decode    = []
    times_total         = []
    peak_mems           = []
    ...
    for idx, image_name in enumerate(tqdm(image_names, desc="Profiling")):
        ...
        # --- AccSamp (full, for total timing) ---
        # Also time each internal phase by using a instrumented wrapper
        phase1_ms_ref  = [0.0]
        align_ms_ref   = [0.0]
        phase2_ms_ref  = [0.0]

        _orig_accsamp = sampler.__class__.accsamp

        def _instrumented_accsamp(self_s, model, device, steps, x_size,
                                   cond, uncond, cfg_scale, cond_fn,
                                   hazy, diffusion, x_T=None, progress=True,
                                   progress_leave=True, proportions=None):
            """Thin wrapper that records phase-level timings."""
            import numpy as _np
            ...
            self_s.make_schedule(steps)
            self_s.to(device)
            if x_T is None:
                x_T = torch.randn(x_size, device=device, dtype=torch.float32)

            x = x_T
            timesteps = _np.flip(self_s.timesteps)
            total_steps = len(self_s.timesteps)
            iterator = tqdm(timesteps, total=total_steps,
                            leave=progress_leave, disable=not progress)
            bs = x_size[0]
            estimate = None

            # ── Phase 1 ────────────────────────────────────────────────
            with CudaTimer(device) as _t1:
                with record_function("accsamp_phase1"):
                    for i, step in enumerate(iterator):
                        ...
                        cur_cfg = self_s.get_cfg_scale(cfg_scale, step)
                        if (i + 1) / len(iterator) < round(1.0 - proportions[0], 2):
                            x = self_s.p_sample(
                                model, x, model_t, t, cond, uncond, cur_cfg,
                                cond_fn=None, return_pred=False
                            )
                        elif (i + 1) / len(iterator) == round(1.0 - proportions[0], 2):
                            pred_x0 = self_s.p_sample(
                                model, x, model_t, t, cond, uncond, cur_cfg,
                                cond_fn=None, return_pred=True
                            )
                            # ── Align (timed separately) ───────────────
                            with CudaTimer(device) as _ta:
                                with record_function("accsamp_align"):
                            ...
                            align_ms_ref[0] = _ta.elapsed_ms
                        else:
                            pass
            phase1_ms_ref[0] = _t1.elapsed_ms

            # ── Phase 2 ────────────────────────────────────────────────
            timesteps2 = _np.flip(self_s.timesteps)
            iterator2  = tqdm(timesteps2, total=total_steps,
                               leave=progress_leave, disable=not progress)
            with CudaTimer(device) as _t2:
                with record_function("accsamp_phase2"):
                    for i, step in enumerate(iterator2):
                        model_t = torch.full((bs,), step, device=device, dtype=torch.long)
                        t       = torch.full((bs,), total_steps - i - 1, device=device, dtype=torch.long)
                        cur_cfg = self_s.get_cfg_scale(cfg_scale, step)
                        if i / len(iterator2) == round(1.0 - proportions[1], 2):
                            x = diffusion.q_sample(
                                x_start=estimate, t=model_t,
                                noise=torch.randn_like(estimate)
                            )
                            x = self_s.p_sample(
                                model, x, model_t, t, cond, uncond, cur_cfg, cond_fn=cond_fn
                            )
                        elif i / len(iterator2) > round(1.0 - proportions[1], 2):
                            x = self_s.p_sample(
                                model, x, model_t, t, cond, uncond, cur_cfg, cond_fn=cond_fn
                            )
                        else:
                            pass
            phase2_ms_ref[0] = _t2.elapsed_ms

            return x

        # Temporarily patch with instrumented version
        sampler.__class__.accsamp = _instrumented_accsamp

        with CudaTimer(device) as t_acc:
            with record_function("accsamp"):
                z = sampler.accsamp(
                    model=cldm,
                    device=device,
                    steps=args.steps,
                    ...
                    diffusion=diffusion,
                    progress=False,
                    proportions=[tau, omega],
                )

        # Restore original
        sampler.__class__.accsamp = _orig_accsamp
        ...
        # Accumulate
        total_ms = t_pre.elapsed_ms + t_cond.elapsed_ms + t_acc.elapsed_ms + t_vae.elapsed_ms
        times_preprocess.append(t_pre.elapsed_ms)
        times_cond.append(t_cond.elapsed_ms)
        times_phase1.append(phase1_ms_ref[0])
        times_align.append(align_ms_ref[0])
        times_phase2.append(phase2_ms_ref[0])
        times_accsamp_total.append(t_acc.elapsed_ms)
    ...
    def stats(arr, label, unit="ms"):
        a = np.array(arr)
        print(f"  {label:<30}  avg={a.mean():8.1f} {unit}"
    ...
    print()
    stats(times_phase1,        "AccSamp phase-1 (w/ align)")
    stats(times_align,         "  └─ Align estimation")
    stats(times_phase2,        "AccSamp phase-2 (guided)")
    stats(times_accsamp_total, "AccSamp total")
    print()
    stats(times_vae_decode,    "VAE decode (final)")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--tau",          type=float, default=None,
                        help="Override cfg.inference.tau (phase-1 proportion).")
    parser.add_argument("--omega",        type=float, default=None,
                        help="Override cfg.inference.omega (phase-2 proportion).")
